...
class BouncingMnist():
    ...
    def __init__(self, mnistDataset='mnist.h5', mode='standard', transform=None, background='zeros', num_frames=20,
                 batch_size=1, image_size=64, num_digits=2, step_length=0.1):
        self.mode_ = mode
    ...
    def get_batch(self, verbose=False):
        ...
        if self.background_ == 'zeros':
            data = np.zeros(
                (self.batch_size_, self.num_channels_, self.image_size_, self.image_size_, self.seq_length_),
                dtype=np.float32)
        elif self.background_ == 'rand':
            data = np.random.rand(self.batch_size_, self.num_channels_, self.image_size_, self.image_size_,
                                  self.seq_length_)
        ...
        for j in range(self.batch_size_):
            for n in range(self.num_digits_):
                ...
                for i in range(self.seq_length_):
                    ...
                    data[j, :, top:bottom, left:right, i] = self.Overlap(data[j, :, top:bottom, left:right, i],
                                                                         digit_image)
